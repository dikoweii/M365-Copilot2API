...
| `M365_MCP_PUBLIC_URL` | 空 | 原生 MCP 工具发现使用的网关公网 HTTPS 根地址，例如 `https://m365.example.com`；带工具请求必须配置，且不会信任客户端 `Host` |
...
1. **显式会话（`X-M365-Session-Id`）**：请求头显式指定的会话 ID 优先级最高，但只在当前已认证 API Key 的租户作用域内解析；不同 Key 使用相同 ID 也不会共享云端对话。
...
- **租户内跨 IP 复用**：内容前缀可在同一 API Key 租户内复用；API Key、显式 session、`user` 和内容缓存共同参与隔离，不同调用方不会因相同短消息或会话 ID 串到一起。
